...
The former ``repo-sentinel: ignore`` spelling remains valid indefinitely so a
rename cannot silently reactivate findings that a project already reviewed.
...
LEGACY_LINE_MARKER = "repo-sentinel: ignore"
...
def marker(line: str) -> "tuple[str, frozenset] | None":
    ...
    if "bluerayscan" not in line and "repo-sentinel" not in line:
        return None
