...
1. Pick the next free id in the right family: `SEC` for secrets, `WF` for
   workflows, `DK` for Dockerfiles. `SEC100+` is reserved for heuristics and
   `SEC900+` for the scanner's own hygiene checks.
...
4. Extend `tests/corpus.py` so the rule fires there. The corpus is what proves
   the catalogue and the scanners agree; a rule missing from it fails the build
   in both directions.
